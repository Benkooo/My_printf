...
{
	print_int, print_int, print_char, print_string, print_string,
	print_unsigned_int, print_hexa, print_hexa_caps, print_octal,
	print_binary, print_percent, print_pointer_hexa, print_long,
	print_longlong
};
...
static int get_flag_or_not(char *str, int i)
{
	char *flags = "%dicsSuxXobp";
	char *parameters = "#+-0123456789";
	int x = 0;

	for (x; str[i] != flags[x]; x++)
		return (1);
	for (int j = 0; str[i] != parameters[j]; j++)
		for (x; str[i] != flags[x]; x++)
			return (1);
	return (0);
}
